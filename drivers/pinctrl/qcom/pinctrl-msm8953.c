...
static struct msm_pinctrl_soc_data msm8953_pinctrl = {
	.pins = msm8953_pins,
...
static int msm8953_pinctrl_gpio_irq_map_probe(struct platform_device *pdev)
{
	...
	msm8953_pinctrl.wakeirq_map = gpio_irq_map;
	msm8953_pinctrl.nwakeirq_map = gpio_irq_map_count;
...
static int msm8953_pinctrl_probe(struct platform_device *pdev)
{
	...
	if (of_find_property(pdev->dev.of_node, "qcom,gpio-irq-map", &len)) {
		ret = msm8953_pinctrl_gpio_irq_map_probe(pdev);
		if (ret) {
			dev_err(&pdev->dev,
